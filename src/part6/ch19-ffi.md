...
pub extern "C" fn greet(name: *const c_char) -> *mut c_char {
    ...
    CString::new(result).unwrap().into_raw()   // 把所有权交给 C
}
...
**重要原则**:**谁分配谁释放**。不能 Rust 用 `malloc` C 用 `free`(allocator 不同)。所以必须暴露 `free_string` 给 C 调用。
...
Rust 的 `&str` 必须是有效 UTF-8。从 C 接来 `*const c_char` 时,**先 `from_ptr().to_str()?` 校验**,否则可能 UB。
