# 第 3 章 · Borrowing 与 Lifetime —— 编译器是结对程序员
...
(回顾 Ch 2:这里更地道的写法是 `&[String]` 而不是 `&Vec<String>`。但本章为了讲清借用本身,我们暂时用前者。)
...
回到你的 Stiglab 代码。找一段你写的或读过的 Rust 代码,标出每个 `&` 引用的 lifetime 来源:它从哪里借来,什么时候失效?
...
我们会用一个具体例子贯穿:**重新设计 Stiglab 的 Session 状态机**,从你熟悉的"struct + status 字段"风格,变成 Rust 风格的"enum 即状态机"。
