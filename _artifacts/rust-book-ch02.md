...
// 选项 2:返回引用,零拷贝
fn filter_long<'a>(strs: &'a [String]) -> Vec<&'a String> {
    strs.iter()
        .filter(|s| s.len() > 3)
        .collect()
}
...
回到 Stiglab Control Plane。设想以下场景:

> 一个 Stiglab Session 持有大量配置(`SessionConfig`)。这个配置被多个组件读取:scheduler 用来决定调度、reporter 用来输出日志、metrics collector 用来打 tag。
...
在什么场景下你会选哪个?**写下你的判断**,在你后面真的接触 Stiglab 这块代码时回头看自己的判断准不准。
...
> **本章一句话总结**
>
> Ownership 不是 Rust 拍脑袋发明的限制,是"无 GC 也要内存安全"这个目标推导出的最小约束。理解了动机,规则就不再是枷锁。
